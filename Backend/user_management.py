...
def register_user():
    ...
    if conn:
        try:
            ...
            cursor.execute("""
                INSERT INTO TFC_user_info_table (id, name, age, weight, height)
                VALUES (?, ?, ?, ?, ?)
            """, (next_id, name, age, weight, height))
...
def login_user():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT u.name, u.age, u.weight, u.height
            ...
            if user_data:
                ...
                print(f"Weight: {user_data[2]} lbs")
                print(f"Height: {user_data[3]}")
            else:
