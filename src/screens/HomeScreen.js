...
import { safeNavigate } from "../shims/NavigationWeb";
...
const HomeScreen = ({ route, navigation }) => {
  ...
  const renderMuscleGroupSection = useCallback(
    (muscles, title) => (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <MuscleGroupItem
              ...
              style={[
                styles[`${getStatus(muscleData[item]?.days || 0)}Status`],
                selectedMuscles.includes(item) && styles.selectedMuscleGroup
              ]}
            />
          )}
  ...
  const getNextWorkoutTime = () => {
    ...
    if (
      !latestWorkout ||
      !latestWorkout.muscles ||
      latestWorkout.muscles.length === 0
    ) {
      return {
    ...
    if (longestRecovery.timeLeft > 0) {
      ...
      const recoveryTimeFormatted = formatTimeLeft(
        longestRecovery.recoveryTime
      );
  ...
  const readyMusclesCount = useMemo(() => {
    ...
    return muscleGroups.filter((muscle) => {
      ...
      const recoveryTime =
        muscleData?.recoveryTime || recoveryTimes[muscle] || 48;
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log(
        "Home screen focused - refreshing muscle data from workout history"
      );
      loadMuscleData();
      return () => {};
  ...
  useEffect(() => {
    if (
      ...
      if (
        lastWorkout &&
        Array.isArray(lastWorkout.muscles) &&
        lastWorkout.muscles.length > 0
      ) {
        setSelectedMuscles(lastWorkout.muscles);
        console.log(
          "DEBUG: Restored selectedMuscles from workoutHistory",
          lastWorkout.muscles
        );
      }
    }
    ...
    if (route.params?.workoutJustSaved) {
      ...
      const loadAndSetWorkout = async () => {
        try {
          ...
          setWorkoutTimer(0);
          console.error(
            "[HomeScreen] Error loading savedWorkout from AsyncStorage",
            e
          );
        }
  ...
  useEffect(() => {
    if (route.params?.workoutConfig) {
      ...
      AsyncStorage.setItem(
        "savedWorkout",
        JSON.stringify(route.params.workoutConfig)
      );
      setWorkoutInProgress(true);
      setIsTimerRunning(true);
      setWorkoutTimer(0);
      console.log(
        "[HomeScreen] Workout configured and started:",
  ...
  useEffect(() => {
    if (route.params?.workout) {
      ...
      AsyncStorage.setItem(
        "savedWorkout",
        JSON.stringify(route.params.workout)
      );
      setWorkoutInProgress(true);
      setIsTimerRunning(true);
      setWorkoutTimer(0);
      console.log(
        "[HomeScreen] Workout loaded and started:",
        route.params.workout
      );
  ...
  const loadWorkoutHistory = async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.workouts)) {
        ...
        const sortedWorkouts = response.data.workouts.sort(
      ...
      if (workoutHistory.length > 0) {
            ...
            workoutMuscles.forEach((muscleGroup) => {
              ...
              if (
                ...
                new Date(workout.end_time) >
                  new Date(muscleLastWorkout[muscleKey].timestamp)
              ) {
                muscleLastWorkout[muscleKey] = {
                ...
                console.log(
                  `Found ${muscleKey} workout on ${new Date(
                    workout.end_time
                  ).toLocaleDateString()} in workout: ${workout.workout_name}`
                );
              }
            });
          }
        ...
        Object.keys(muscleLastWorkout).forEach((muscle) => {
          ...
          if (muscleDataObj.hasOwnProperty(muscle)) {
            ...
            console.log(
              `Updated ${muscle} to ${diffDays} days based on workout history`
            );
          }
        });
  ...
  const startWorkout = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Navigation Error",
        "Unable to navigate to the workout configuration screen. Please try again."
      );
    }
  };
  ...
  const endWorkout = async () => {
    ...
    if (!workoutToSave) {
      Alert.alert(
        "No saved workout",
        "You must start and save a workout before ending it."
      );
      console.error(
        "No savedWorkout found in state or AsyncStorage. Cannot end workout."
      );
      return;
    }
    ...
    if (!Array.isArray(exercises) || exercises.length === 0) {
      Alert.alert(
        "No exercises found",
        "Cannot save a workout with no exercises."
      );
      return;
    }
    ...
    const startTime =
      workoutToSave?.startTime ||
      workoutToSave?.startedAt ||
      new Date(Date.now() - workoutTimer * 1000).toISOString();
    ...
    if (!userId) {
      ...
      console.error(
        "No user_id found in Redux or AsyncStorage. Cannot save workout."
      );
      return;
    }
    ...
    function extractMuscleKeysFromExercise(exercise) {
      ...
      const isCalfExercise =
        ...
        (name.includes("raise") &&
          (name.includes("heel") || name.includes("toe")));
    ...
    if (workoutToSave && Array.isArray(workoutToSave.exercises)) {
      workoutToSave.exercises.forEach((exercise) => {
        extractMuscleKeysFromExercise(exercise).forEach((muscle) =>
    ...
    if (selectedMuscles && selectedMuscles.length > 0) {
      selectedMuscles.forEach((muscle) =>
        musclesWorked.add(muscle.toLowerCase())
      );
    }
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5001/api/closed_workouts",
        {
          user_id: userId,
    ...
    const reduxCoreExercise =
      workoutToSave.exercises &&
      Array.isArray(workoutToSave.exercises) &&
      workoutToSave.exercises.some(
        (ex) =>
          ex.name &&
    ...
    if (
      reduxCoreExercise &&
      !muscleArrayLower.includes("core") &&
      !muscleArrayLower.includes("abs")
    ) {
    ...
    if (muscleArrayFinal.length > 0) {
      ...
      if (
        ...
        console.log(
          "Core/abs workout detected: triggering core timer update in Redux with endTime:",
          endTime
        );
      }
      // --- END NEW ---
      ...
      console.log(
        "Current muscle data before reset:",
        JSON.stringify(newMuscleData)
      );
      ...
      const hasCalfExercise =
        workoutToSave.exercises &&
        Array.isArray(workoutToSave.exercises) &&
        workoutToSave.exercises.some((ex) => {
      ...
      if (hasCalfExercise) {
        ...
        if (newMuscleData.hasOwnProperty(muscleKey)) {
          console.log(
            `Resetting ${muscleKey} from ${newMuscleData[muscleKey]} days to 0 days`
          );
      ...
      console.log(
        "Updated muscle data after reset:",
        JSON.stringify(newMuscleData)
      );
    }
    ...
    const hasCompoundLegExercise =
      ...
      workoutToSave.exercises.some((ex) => {
        ...
        return (
          ...
          desc.includes("leg press") ||
          name.includes("hack squat") ||
          desc.includes("hack squat") ||
          name.includes("lunge") ||
          desc.includes("lunge") ||
          name.includes("step up") ||
          desc.includes("step up") ||
          name.includes("deadlift") ||
          desc.includes("deadlift") ||
          name.includes("bulgarian split") ||
          desc.includes("bulgarian split") ||
          name.includes("leg day") ||
          desc.includes("leg day")
        );
      });
      ...
      workoutToSave.exercises.some((ex) => {
        ...
        return (
          (name.includes("quad") ||
            ...
            desc.includes("leg extension") ||
            name.includes("sissy squat") ||
            desc.includes("sissy squat")) &&
          !hasCompoundLegExercise
        ); // Don't double count if compound exercises exist
      });
    if (hasQuadOnlyExercise) affectedMuscles.push("Quads");

    // Check for chest exercises
    const hasChestExercise =
      workoutToSave.exercises &&
      Array.isArray(workoutToSave.exercises) &&
      workoutToSave.exercises.some((ex) => {
        const name = (ex.name || "").toLowerCase();
        ...
        return (
          ...
          desc.includes("bench press") ||
          name.includes("push-up") ||
          name.includes("pushup") ||
          desc.includes("push-up") ||
          desc.includes("pushup") ||
          name.includes("fly") ||
          desc.includes("fly") ||
          name.includes("dip") ||
          desc.includes("dip") ||
    ...
    const hasBackExercise =
      ...
      workoutToSave.exercises.some((ex) => {
        ...
        return (
          ...
          desc.includes("pulldown") ||
          name.includes("pullup") ||
          desc.includes("pullup") ||
          name.includes("chin up") ||
          desc.includes("chin up") ||
          name.includes("deadlift") ||
          desc.includes("deadlift")
        );
      });
    ...
    const hasTricepsExercise =
      ...
      workoutToSave.exercises.some((ex) => {
        ...
        return (
          name.includes("tricep") ||
          desc.includes("tricep") ||
          name.includes("extension") ||
          desc.includes("extension") ||
          name.includes("pushdown") ||
          desc.includes("pushdown") ||
          name.includes("kickback") ||
          desc.includes("kickback") ||
          name.includes("skull crusher") ||
          desc.includes("skull crusher") ||
          name.includes("close grip") ||
          desc.includes("close grip") ||
          name.includes("overhead extension") ||
          desc.includes("overhead extension")
        );
      });
  ...
  const quickActions = useMemo(() => {
    const handleStartWorkout = () => {
      try {
        console.log(
          "[HomeScreen] Start Workout button pressed - calling startWorkout function"
        );
        startWorkout();
      } catch (e) {
        ...
        Alert.alert(
          "Error",
          "There was an error starting the workout. Please try again."
        );
      }
    };
  ...
  return (
    ...
    >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            try {
              ...
              console.error(
                "Navigation error to Calculator (ready muscles):",
                e
              );
            }
          }}
        >
          <Text style={styles.statNumber}>{readyMusclesCount}</Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            try {
              ...
              console.error(
                "Navigation error to Calculator (resting muscles):",
                e
              );
            }
          }}
        >
          <Text style={styles.statNumber}>{restingMusclesCount}</Text>
          ...
          styles.nextWorkoutContainer,
          {
            backgroundColor:
              nextWorkoutInfo.status === "ready"
                ...
                ? "#f59e0b"
                : nextWorkoutInfo.status === "inProgress"
                ? "#f59e0b"
                : "#ef4444",
            borderLeftWidth: 6,
            borderLeftColor:
      ...
      >
        ...
        {nextWorkoutInfo.lastWorkout && (
          <Text style={styles.lastWorkoutText}>
            Last workout:{" "}
            {new Date(nextWorkoutInfo.lastWorkout).toLocaleString()}
          </Text>
        )}
      </View>
          ...
          <View style={styles.configuredWorkoutSection}>
            ...
            <View style={{ marginBottom: 8 }}>
              {savedWorkout.exercises.map((ex, idx) => (
                <View key={idx} style={styles.configuredWorkoutExerciseCard}>
                  <Text style={styles.configuredWorkoutExerciseName}>
                    {ex.name}
                  </Text>
                  <Text style={styles.configuredWorkoutExerciseDesc}>
                    {ex.description}
                  </Text>
                      ...
                      {ex.sets.map((set, setIdx) => (
                        ...
                        >
                          <Text style={styles.configuredWorkoutSetText}>
                            Set {setIdx + 1}:{" "}
                            {set.setType.charAt(0).toUpperCase() +
                              set.setType.slice(1)}{" "}
                            | Reps: {set.reps}{" "}
                            {set.weight ? `| Weight: ${set.weight}` : ""}
                          </Text>
                          {set.notes && (
                            <Text style={styles.configuredWorkoutSetNotes}>
                              Notes: {set.notes}
                            </Text>
                          )}
                        </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                try {
                  console.log(
                    "[HomeScreen] Safely navigating to ConfigureWorkout from Edit button"
                  );
                  safeNavigate(navigation, "ConfigureWorkout", {
                  ...
                  console.error(
                    "[HomeScreen] Edit button navigation error:",
                    e
                  );
                  Alert.alert(
                    "Navigation Error",
            ...
            >
              <Text style={styles.configuredWorkoutEditButtonText}>
                Edit Workout
              </Text>
            </TouchableOpacity>
          </View>
      ...
      >
        ...
        >
          <Ionicons name="barbell-outline" size={20} color="#ffffff" />
          <Text style={[styles.buttonText, { fontSize: 15, marginLeft: 6 }]}>
            Start Workout
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={() => {
            console.log(
              "End Workout button was pressed. workoutInProgress:",
              workoutInProgress
            );
            if (workoutInProgress) {
              endWorkout();
              ...
              Alert.alert(
                ...
                "Start a workout before ending it."
              );
            }
          }}
        ...
        >
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => setIsWorkoutOptionsVisible(false)}
            >
              <Text
                style={{
                  color: "#4b2e83",
      ...
      >
        <View
          style={{
            ...
            alignItems: "center",
            backgroundColor: "rgba(20,22,34,0.0)"
          }}
        >
          <View
            style={{
          ...
          >
            ...
            >
              <Text
                style={{ color: "#6b46c1", fontWeight: "600", fontSize: 18 }}
              >
                Continue
              </Text>
            </TouchableOpacity>
