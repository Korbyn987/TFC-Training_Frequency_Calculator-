...
const SelectRoutineScreen = ({ navigation }) => {
    ...
    const fetchPresets = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setPresets(data[0].workout_presets || []);
        }
      } catch (err) {
        console.error(err);
        setPresets([]);
  ...
  const handleSelectPreset = (preset) => {
    ...
    const formattedExercises = preset.exercises.map((exercise) => ({
      ...
      id:
        exercise.id ||
        exercise.name.replace(/\s+/g, "_").toLowerCase() + "_" + Date.now(),
        ...
        {presets.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Create presets in your profile to use them here.
            </Text>
          </View>
        ) : (
          <FlatList
            ...
            renderItem={({ item, index }) => {
              ...
              const cardStyle = [
                ...
                index % 3 === 0
                  ? styles.purpleCard
                  : index % 3 === 1
                  ? styles.darkPurpleCard
                  : styles.brightPurpleCard
              ];
