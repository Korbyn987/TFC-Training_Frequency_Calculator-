...
const WorkoutPresets = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const addExerciseActive =
        state.routes[state.routes.length - 1]?.name === "AddExercise";
      if (
        profileRoute &&
        profileRoute.params &&
        ...
        setTimeout(() => {
          ...
          navigation.setParams({
            selectedExercisesForPreset: undefined,
            showPresetModal: undefined
          });
        }, 350);
      }
    ...
    if (editingPreset) {
      newPresets = presets.map((p) =>
        p.id === editingPreset.id
          ? { ...p, name, exercises: presetExercises }
          : p
      );
    } else {
      newPresets = [
  ...
  const handleLoadPreset = (preset) => {
    // Navigate to WorkoutOptionsScreen with the preset data
  ...
  const handleAddExerciseNav = () => {
    ...
    setTimeout(() => {
      setPendingPreset((prev) =>
        prev ? { ...prev, pendingAddExercise: false } : prev
      );
  ...
  return (
    <View style={styles.section}>
        ...
        renderItem={({ item }) => (
          <View>
            <TouchableOpacity
              style={[
                styles.presetCard,
                expandedPresetId === item.id && styles.presetCardActive
              ]}
              onPress={() =>
                setExpandedPresetId(
                  expandedPresetId === item.id ? null : item.id
                )
              }
              activeOpacity={0.85}
            >
              ...
              <Ionicons
                name={
                  expandedPresetId === item.id ? "chevron-up" : "chevron-down"
                }
                size={22}
                color="#6b46c1"
                ...
                <View style={styles.exerciseList}>
                  {item.exercises.length === 0 ? (
                    ...
                    item.exercises.map((e, idx) => (
                      <View
                        key={e.id || e.name + idx}
                        style={styles.exerciseRowExpanded}
                      >
                        <Ionicons
                          name="ellipse"
                          size={8}
                          ...
                          style={{ marginRight: 8 }}
                        />
                        <Text style={styles.exerciseNameExpanded}>
                          {e.name}
                        </Text>
                      </View>
                    ))
      ...
      <Modal
        visible={modalVisible && !isAddExerciseScreenActive()}
        animationType="slide"
        transparent={true}
          ...
          <View style={styles.modalContent}>
            ...
            >
              ...
              <View style={styles.saveCancelRow}>
                ...
                {editingPreset && (
                  <TouchableOpacity
                    style={styles.deleteBtnPopup}
                    onPress={() => {
                      handleDelete(editingPreset.id);
                      closeModal();
                    }}
                  >
